...
export default function ReportGenerator() {
  ...
  const generateReport = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://127.0.0.1:5000/reports/generate-from-input", { // Ensure the correct API route
        method: "POST",
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      {report && (
        <div className="mt-6 p-4 bg-gray-100 border rounded-md w-full">
